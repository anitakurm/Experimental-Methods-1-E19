...
Once you read in your data, you can use head(data) to quickly inspect data by seeing the first several values from it.

```{r}
#inspect the first part of data using head(data)
head(child)
```
...
Already now you should try to answer the following questions:
  ...
  What kind of statistical tests can you use to explore this kind of data? Could you run a t-test?
    ...
    - no categorical variables -> can't run a t-test without making a categorical variable first

### Part 1: Simple linear regression

#### lm(formula, data) and summary()
...
Alternatively, we can draw a regression line automatically using geom_smooth(method = lm)
...
```{r}
#making 5 regressions
m1 <- lm(Aggression ~ Television, child)
m2 <- lm(Aggression ~ Computer_Games, child)
m3 <- lm(Aggression ~ Diet, child)
m4 <- lm(Aggression ~ Sibling_Aggression, child)
m5 <- lm(Aggression ~ Parenting_Style, child)
...
ggplot(child, aes(Computer_Games, Aggression)) + 
  geom_point() + 
  geom_smooth(method = lm, se=FALSE)

ggplot(child, aes(Diet, Aggression)) + 
  geom_point() + 
  geom_smooth(method = lm, se=FALSE)


ggplot(child, aes(Sibling_Aggression, Aggression)) + 
  geom_point() + 
  geom_smooth(method = lm, se=FALSE)


ggplot(child, aes(Parenting_Style, Aggression)) + 
  geom_point() + 
  geom_smooth(method = lm, se=FALSE)
...
#check cor()
cor(child)
round(cor(child),2) #much better! but still hard...
```
...
```{r}
#1
mm1 <- lm(Aggression ~ Sibling_Aggression, child)

#2
mm2 <- lm(Aggression ~ Sibling_Aggression + Computer_Games, child)

#3
mm3 <- lm(Aggression ~ Sibling_Aggression + Computer_Games + Parenting_Style, child)
...
1a) Play with geom_smooth, try running without the method argument
...
ggplot(child, aes(Television, Aggression )) + 
  ...
  geom_smooth(method = lm, aes(Computer_Games, color = 5))+
  geom_smooth(method = lm, aes(x = Sibling_Aggression, color = 4))+
  geom_smooth(method = lm, aes(x = Computer_Games, color = 3))+
  geom_smooth(method = lm, aes(x = Diet, color = 2))+
  geom_smooth(method = lm, aes(x = Parenting_Style, color = 1))
```
...
(didn't have time to figure it out, will add solution later)
```{r}
```
